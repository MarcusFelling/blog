...
<figure class="wp-block-image size-large is-style-default">![](https://test.local/wp-content/uploads/2020/04/AssociatedAutomation.png)<figcaption>*Example of a manual test case with no automation*</figcaption></figure>  
...
<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/CreateRelease.png)</figure>Add the build artifact that contains the automated tests:

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/AddArtifact.png)</figure>Add 2 tasks to a stage. In my example I’m using my “Test” environment for the stage. The first task will install the Visual Studio Test Platform. The second task will execute the tests using VsTest. Set the second task’s platform version to “Installed by Tools Installer” so you don’t need to manually install anything on the environment/server that’s running the tests. We’ll be triggering these tests through a Test Case, so set the “Select tests using” field to “Test Run”.

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/Tasks.png)</figure>## Configuring Azure Test Plans

Next, we can configure our Test Plan to use our build and release definitions. This can be done in Test Plan settings:

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/TestPlanSettings.png)</figure><figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/TestPlanSettingBuildReleases.png)</figure>We can now execute automated tests in our Test Plan using our automated tests pipeline.
...
<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/AssociateTestCase.png)</figure>Find the Test Case:

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/FindTestCase.png)</figure>After saving, you can then find the fields in the Test Case’s Associated Automation tab populated:

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/TestCaseFieldsPopulated.png)</figure>## Running the tests

Finally, testers can now trigger the automated Test Case!

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/Trigger1-1024x233.png)</figure><figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/Trigger2.png)</figure>Then see the results in Test Runs:

<figure class="wp-block-image size-large">![](https://test.local/wp-content/uploads/2020/04/RunResults.png)</figure>Happy testing!
