...
<figure class="wp-block-image">![](https://test.local/wp-content/uploads/2019/02/SonarQubeServiceConnection.png)</figure>## Azure Pipelines Configuration
...
<figure class="wp-block-image">![](https://test.local/wp-content/uploads/2019/02/PrepareAnalysisSonarQubeBuild-1024x577.png)</figure>2\. Add the task: **Run Code Analysis**

You will want to add this task towards the end of the build.

<figure class="wp-block-image">![](https://test.local/wp-content/uploads/2019/02/RunCodeAnalysisSonarQube.png)</figure>3\. Add the task: **Publish Quality Gate Result** (Optional)
...
<figure class="wp-block-image">![](https://test.local/wp-content/uploads/2019/02/PublishQualityGateResultSonarQubeBuild.png)</figure>Our build definition is now ready. You will notice that adding the new SonarQube tasks will add a new demand, requiring Java as as an agent capability. This means you will need to install Java on your build server if you’re using a Private agent/pool.

<figure class="wp-block-image">![](https://test.local/wp-content/uploads/2019/02/JavaDemandSonarQube-1024x562.png)</figure>3\. Time to queue the build!

## SonarQube

After running the build with the SonarQube tasks, you should now see a new project in SonarQube.

<figure class="wp-block-image">![](https://test.local/wp-content/uploads/2019/02/BlogSonarQubeExample-1024x141.png)</figure>That’s all for now folks…