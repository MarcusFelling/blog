...
---
AI makes it easy to ask a data question in plain English and get an answer in seconds. The hard part is knowing whether that answer used the right model, measure, filters, and time period. [Vibe analytics](https://mitsloan.mit.edu/ideas-made-to-matter/working-definitions/what-is-vibe-analytics) is the tempting shortcut: ask, accept the result, and move on. It borrows the posture of [vibe coding](https://en.wikipedia.org/wiki/Vibe_coding), Andrej Karpathy's term for letting AI write the code and accepting it without reading the diffs.

So I built an AI analyst that keeps the plain-English interface but makes the result verifiable. It can answer a focused question from the right governed semantic model or connect evidence across the data landscape for a broader briefing. Each query runs under the user's identity, and the answer includes its sources, periods, and filters. Ask for the DAX and it returns the exact query too. The LLM is the interface. The real product is the institutional knowledge that defines what a correct answer requires.
